...
def gen_1m_train_test(path,fold):
    ...
    for user in user_historicals:
        ...
        if len(user_historicals[user]) < filter_nb:
            continue
...
def cold_train(file, test_num, warm_split, k):
    ...
    for epoch in range(nb_epcoch):
        ...
        for j, user in enumerate(train_users):
            ...
            while end_slice <= overall_length and start_slice < overall_length:
                ...
                if epoch > swith_epoch:
                    rein, train_hit, final_state, masking, samples = im.reinforcement_learn(interest, masking,
                                                                                            inference_length,
                                                                                            final_state, s_token,
                                                                                            s_hit)
                else:
                    sup, train_hit, final_state, masking, samples = im.supervised_learn(interest, masking,
                                                                                        slice_items,
                                                                                        inference_length,
                                                                                        final_state, s_token, s_hit)
        ...
        for user in test_users:
            ...
            (user_item_probs, user_selected_items, user_final_masking, user_hit, user_imediate_reward,
             user_cumsum_reward) = im.inference(interest, masking, inference_length, final_state)
            ...
            user_selected_items = np.reshape(user_selected_items,(inference_length,))
